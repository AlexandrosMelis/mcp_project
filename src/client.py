...
server_params = StdioServerParameters(
    ...
    args=["run", "src/tools.py"],  # Command line arguments
...
async def run():
    ...
    async with stdio_client(server_params) as (
        read,
        write,
    ):  # setup context manager, pass the params of server and establish a connection as subprocess
        # once established, we can get access to read and write stream that we can pass to the client session
